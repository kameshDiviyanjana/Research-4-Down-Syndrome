// src/features/writing/src/components/ShapeAnimator.jsx
import React, { useEffect, useRef, useState, forwardRef, useImperativeHandle } from 'react';
import { gsap } from 'gsap';
import PropTypes from 'prop-types';

// This component will animate an SVG shape by drawing its paths
const ShapeAnimator = forwardRef(({ 
  SvgComponent, 
  animationDelay = 0.5, 
  duration = 3,
  tracerColor = "#FF5733",
  ...
  useEffect(() => {
    ...
    xhr.onload = function() {
      if (xhr.status === 200) {
        ...
        // Apply appropriate styling (similar to ShapeDisplay.jsx)
        ...
        paths.forEach((path, index) => {
          ...
          const pathDuration = Math.min(Math.max(pathLength / 100, 1), 4); // Duration based on path length
  ...
  if (!SvgComponent) {
    return <div>No shape to animate</div>;
  }
...
ShapeAnimator.displayName = 'ShapeAnimator';

ShapeAnimator.propTypes = {
